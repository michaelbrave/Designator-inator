defmodule DesignatorInator.Tools.WorkspaceTest do
  @moduledoc """
  Tests for `DesignatorInator.Tools.Workspace`.
  ...
  alias DesignatorInator.Tools.Workspace
  ...
  describe "delete_file/2" do
    test "deletes an existing file", %{workspace: ws} do
      path = Path.join(ws, "deleteme.txt")
      File.write!(path, "gone soon")

      assert {:ok, "Deleted: deleteme.txt"} = Workspace.delete_file(ws, "deleteme.txt")
      refute File.exists?(path)
    end

    test "returns error for missing file", %{workspace: ws} do
      assert {:error, msg} = Workspace.delete_file(ws, "missing.txt")
      assert msg =~ "not found"
    end
  end
  ...
  describe "call/1" do
    ...
    test "returns error when workspace root is missing" do
      assert {:error, "Missing workspace root"} =
               Workspace.call(%{"action" => "list"})
    end
  end
end
