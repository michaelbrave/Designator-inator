defmodule ForgeClaw.PodSupervisor do
  @moduledoc """
  ...
  - Track running pods by name (via `ForgeClaw.PodRegistry`)
  - Gracefully stop pods (allow in-flight requests to complete)

  ## Process structure

  When a pod is started, the supervisor adds a child spec for `ForgeClaw.Pod`.
  Each pod registers itself in `ForgeClaw.PodRegistry` under its name on init.

      ForgeClaw.PodSupervisor (DynamicSupervisor)
          │
          ├── ForgeClaw.Pod (name: "assistant")
          ├── ForgeClaw.Pod (name: "code-reviewer")
          └── ForgeClaw.Pod (name: "orchestrator")
  ...
  3. Spawn a `ForgeClaw.Pod` GenServer under this supervisor

  ## State checkpoint

  Pods persist conversation state to SQLite (via `ForgeClaw.Memory`).  When a
  ...
  3. Start `ForgeClaw.Pod` as a supervised child
  4. Return `{:ok, pid}` when the pod is successfully started (not necessarily ready)

  ## Examples

      iex> ForgeClaw.PodSupervisor.start_pod("./examples/assistant/")
      {:ok, #PID<0.456.0>}

      iex> ForgeClaw.PodSupervisor.start_pod("./nonexistent/")
      {:error, :enoent}

      iex> ForgeClaw.PodSupervisor.start_pod("./bad-pod/")
  ...
  def start_pod(path) do
    ...
    # 6. child_spec = {ForgeClaw.Pod, path: path, manifest: manifest}
  ...
  ## Examples
      ...
      iex> ForgeClaw.PodSupervisor.stop_pod("assistant")
      :ok

      iex> ForgeClaw.PodSupervisor.stop_pod("nonexistent")
      {:error, :not_found}
  """
  ...
  ## Examples
      ...
      iex> ForgeClaw.PodSupervisor.list_pods()
      [
  ...
  def list_pods do
    # Template (HTDP step 4):
    # 1. Registry.select(ForgeClaw.PodRegistry, [{:"$1", [], [:"$1"]}]) → list of {name, pid, _}
