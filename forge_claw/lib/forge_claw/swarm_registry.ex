defmodule ForgeClaw.SwarmRegistry do
  @moduledoc """
  Cross-node pod discovery using Erlang's `:pg` (process groups).

  ## How Erlang distribution works here (HTDP step 1)

  When two ForgeClaw nodes connect via `Node.connect/1`, Erlang's distribution
  ...
  Nodes must:
  1. Share the same Erlang cookie (configured in `~/.forgeclaw/config.yaml`)
  2. Be reachable on the LAN

  Connect with: `Node.connect(:"forgeclaw@192.168.1.50")` via `forge connect <ip>`.
  ...
  alias ForgeClaw.Types.NodeInfo

  @pg_scope :forgeclaw_swarm
  ...
  Called by `ForgeClaw.Pod` in its `init/1`.  Works identically whether the
  pod is local or (in theory) calling from a remote node.

  ## Examples

      iex> ForgeClaw.SwarmRegistry.register_pod("assistant", self())
      :ok
  """
  ...
  Called by `ForgeClaw.Pod` in its shutdown handler.

  ## Examples

      iex> ForgeClaw.SwarmRegistry.deregister_pod("assistant", self())
      :ok
  """
  ...
  ## Examples
      ...
      iex> ForgeClaw.SwarmRegistry.find_pod("code-reviewer")
      {:ok, {#PID<0.456.0>, :"forgeclaw@pi4.local"}}

      iex> ForgeClaw.SwarmRegistry.find_pod("nonexistent")
      {:error, :not_found}
  """
  ...
  ## Examples
      ...
      iex> ForgeClaw.SwarmRegistry.list_all()
      [
        %{name: "assistant",     pid: #PID<0.123.0>, node: :"forgeclaw@macbook"},
        %{name: "code-reviewer", pid: #PID<...>,     node: :"forgeclaw@pi4"}
      ]
  """
  ...
  ## Examples
      ...
      iex> ForgeClaw.SwarmRegistry.list_on_node(:"forgeclaw@pi4.local")
  ...
  @doc """
  Connects this node to a remote ForgeClaw node at `ip_or_hostname`.

  The remote node must be running ForgeClaw with the same Erlang cookie.

  ## Examples

      iex> ForgeClaw.SwarmRegistry.connect("192.168.1.50")
      {:ok, :"forgeclaw@192.168.1.50"}

      iex> ForgeClaw.SwarmRegistry.connect("unreachable-host")
      {:error, :connect_failed}
  """
  @spec connect(String.t()) :: {:ok, node()} | {:error, :connect_failed}
  def connect(ip_or_hostname) do
    # Template:
    # 1. Build node name: :"forgeclaw@#{ip_or_hostname}"
  ...
  ## Examples
      ...
      iex> ForgeClaw.SwarmRegistry.node_infos()
      [
        %NodeInfo{node: :"forgeclaw@macbook", vram_used_mb: 4096, loaded_models: ["mistral-7b"]},
        %NodeInfo{node: :"forgeclaw@pi4",     vram_used_mb: 2048, loaded_models: ["phi-3-mini"]}
      ]
  """
