defmodule ForgeClaw.Pod do
  @moduledoc """
  GenServer representing one running agent pod.

  ## Lifecycle (HTDP step 1)

  A pod goes through these states (see `ForgeClaw.Types.PodState.pod_status`):
  ...
  Pods are started by `ForgeClaw.PodSupervisor.start_pod/1`, not directly.
  ...
  alias ForgeClaw.Types.{PodState, PodManifest, Message}
  alias ForgeClaw.{ReActLoop, Memory, ModelManager, ToolRegistry}
  alias ForgeClaw.Pod.{Manifest, Config}
  ...
  Not called directly — use `ForgeClaw.PodSupervisor.start_pod/1`.
  ...
  ## Examples
      ...
      iex> ForgeClaw.Pod.chat("assistant", "What files do I have?", nil)
      {:ok, "Your workspace contains: notes.md, plan.md", "session-uuid"}

      iex> ForgeClaw.Pod.chat("assistant", "And what's in notes.md?", "session-uuid")
      {:ok, "The notes file contains...", "session-uuid"}

      iex> ForgeClaw.Pod.chat("nonexistent", "Hello", nil)
      {:error, :pod_not_found}
  """
  ...
  ## Examples
      ...
      iex> ForgeClaw.Pod.call_tool("code-reviewer", "review_code",
      ...>   %{"code" => "def foo, do: :bar", "language" => "elixir"})
      {:ok, "The code looks good. One suggestion: ..."}

      iex> ForgeClaw.Pod.call_tool("code-reviewer", "nonexistent_tool", %{})
  ...
  ## Examples
      ...
      iex> ForgeClaw.Pod.get_status("assistant")
      {:ok, %ForgeClaw.Types.PodState{status: :idle, model: "mistral-7b", ...}}

      iex> ForgeClaw.Pod.get_status("nonexistent")
      {:error, :pod_not_found}
  """
  ...
  ## Examples
      ...
      iex> ForgeClaw.Pod.halt("assistant")
      :ok
  """
  ...
  def via(pod_name) do
    {:via, Registry, {ForgeClaw.PodRegistry, pod_name}}
  end

  @doc false
  @spec lookup(String.t()) :: {:ok, pid()} | {:error, :not_found}
  def lookup(pod_name) do
    case Registry.lookup(ForgeClaw.PodRegistry, pod_name) do
      [{pid, _}] -> {:ok, pid}
      [] -> {:error, :not_found}
    end
  end
